...
const Certificate = ({ levelId, userName, onDownload, initialLevels }) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        const canvas = await html2canvas(element, { 
            scale: 3, // أعلى جودة ممكنة
            useCORS: true,
            backgroundColor: '#F3F0E9' // لون خلفية الشهادة
        });
        ...
        <div className="p-4 md:p-8 animate-fade-in flex flex-col items-center justify-center z-50 fixed inset-0 bg-slate-900/80 backdrop-blur-sm">
            ...
            >
                ...
                <div className="text-center relative z-10 my-auto">
                    ...
                    <p className="text-6xl font-bold text-[#0D2C54] my-4" style={{ fontFamily: "'Times New Roman', Times, serif" }}>{userName || 'Valued Student'}</p>
                ...
                <div className="flex justify-between items-center relative z-10 pt-4">
                    ...
                    <div className="text-center w-1/3">
                        <p className="text-2xl font-semibold border-b-2 border-slate-400 pb-1 mx-auto max-w-[150px]" style={{ fontFamily: "'Brush Script MT', cursive" }}>Stellar Speak</p>
                        <p className="text-sm mt-1 text-slate-600">Official Signature</p>
                    </div>
                </div>
            </div>
            {/* --- (نهاية التعديل النهائي للشهادة) --- */}
