...
import { BookOpen, Feather, Award, Sun, Moon, FileText, Download, MessageSquare, BrainCircuit, Library, Sparkles, Wand2, ArrowLeft, CheckCircle, LoaderCircle, XCircle, RefreshCw, Mic, Voicemail, Star, History, ShoppingCart, Users, Newspaper } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    try {
        ...
        const savedIsDarkMode = localStorage.getItem('stellarSpeakIsDarkMode');
  ...
  useEffect(() => {
    ...
    try {
        ...
        localStorage.setItem('stellarSpeakIsDarkMode', JSON.stringify(isDarkMode));
    } catch (error) {
        console.error("Failed to save state to localStorage", error);
    }
  }, [page, userLevel, lessonsDataState, isDarkMode, isInitialLoad]);
  ...
  return (
    <>
      ...
      <div className={`relative z-10 min-h-screen font-sans ${isDarkMode ? 'bg-slate-900/80 text-slate-200' : 'bg-sky-100 text-slate-800'}`}>
