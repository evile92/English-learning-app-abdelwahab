...
// --- Gemini API Helper (No changes) ---
async function runGemini(prompt, schema) {
    const apiKey = process.env.REACT_APP_GEMINI_API_KEY;
    if (!apiKey) {
        console.error("Gemini API key is not set! Please set REACT_APP_GEMINI_API_KEY in your environment variables.");
...
// --- Custom Hook for Local Storage (No changes, it's already good) ---
...
// --- المكونات الفرعية (معرفة خارج المكون الرئيسي) (No changes) ---
...
export default function App() {
  ...
  const handleCompleteLesson = (lessonId, score, total) => {
      ...
      // We need to use a timeout to ensure state has updated before checking for completion
      setTimeout(() => {
          const allLessonsInLevel = lessonsDataState[levelId];
          const isLevelComplete = allLessonsInLevel.every(lesson => lesson.completed);
          if (isLevelComplete) {
