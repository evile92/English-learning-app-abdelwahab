...
import { BookOpen, Feather, Award, Sun, Moon, FileText, Download, MessageSquare, BrainCircuit, Library, Sparkles, Wand2, ArrowLeft, CheckCircle, LoaderCircle, XCircle, RefreshCw, Mic, Voicemail } from 'lucide-react';
...
const Certificate = ({ levelId, onDownload }) => {
    return (
        <div className="p-4 md:p-8 animate-fade-in text-center flex flex-col items-center">
            <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-2">تهانينا! 🏆</h1>
            <p className="text-slate-600 dark:text-slate-300 mb-6">لقد أكملت بنجاح متطلبات المستوى {levelId} وحصلت على هذه الشهادة.</p>
            <div className="w-full max-w-2xl aspect-[1.414] bg-white dark:bg-slate-800 border-4 border-sky-500 p-8 rounded-lg shadow-2xl relative">
                <div className="absolute inset-0 bg-repeat bg-center opacity-5" style={{backgroundImage: "url('data:image/svg+xml,...')"}}></div>
                <div className="text-center relative">
                    <Award size={60} className="mx-auto text-amber-500 mb-4" />
                    <p className="text-lg text-slate-600 dark:text-slate-300">Certificate of Achievement</p>
                    <h2 className="text-4xl font-bold text-sky-600 dark:text-sky-400 my-4">Level {levelId} Completion</h2>
                    <p className="text-lg text-slate-600 dark:text-slate-300">This certifies that</p>
                    <p className="text-3xl font-serif text-slate-800 dark:text-slate-100 my-4 border-b-2 border-dotted border-slate-400 pb-2">Your Name Here</p>
                    <p className="text-lg text-slate-600 dark:text-slate-300">has successfully completed all the requirements for Level {levelId} of the English Language Crossing program.</p>
                    <p className="mt-8 text-sm text-slate-500 dark:text-slate-400">Issued on: {new Date().toLocaleDateString()}</p>
                </div>
            </div>
            <button onClick={onDownload} className="mt-8 bg-green-500 text-white font-bold py-3 px-8 rounded-full text-lg hover:bg-green-600 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center gap-2"><Download size={20} /> تحميل والعودة للرئيسية</button>
        </div>
    );
};
